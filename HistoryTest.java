...
public class HistoryTest extends Tests{
    public History hQuestions;
    public int answeredright;
    Question[] questionstobeused;
    public int on;
  
    public HistoryTest(int difficulty) {
	hQuestions = new History();
	questionstobeused = new Question[5];
	if (difficulty == 0)
	    for (int i = 0; i < 5; i++) {
		questionstobeused[i] = hQuestions.hQuestionsLevel0.remove(i);
    }
    if (difficulty ==1) {
      ...
      questionstobeused[0] = hQuestions.hQuestionsLevel1.remove(random0);
      questionstobeused[1] = hQuestions.hQuestionsLevel1.remove(random1);
      questionstobeused[2] = hQuestions.hQuestionsLevel1.remove(random2);
      questionstobeused[3] = hQuestions.hQuestionsLevel0.remove(random3);
      questionstobeused[4] = hQuestions.hQuestionsLevel0.remove(random4);
    }
    if (difficulty ==2) {
      int random0 = (int)(Math.random() * 5);
      int random1 = (int)(Math.random() * 4);
      ...
      questionstobeused[0] = hQuestions.hQuestionsLevel2.remove(random0);
      questionstobeused[1] = hQuestions.hQuestionsLevel2.remove(random1);
      questionstobeused[2] = hQuestions.hQuestionsLevel1.remove(random2);
      questionstobeused[3] = hQuestions.hQuestionsLevel1.remove(random3);
      questionstobeused[4] = hQuestions.hQuestionsLevel0.remove(random4);
    }
    for (int i = 0; i < 5; i++)
	testIt(i);
